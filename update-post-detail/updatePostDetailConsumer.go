...
func initAmqp() {
	...
	err = ch.ExchangeDeclare(
		"update-post-detail-exchange", // name of the exchange
		"direct",                      // type
		true,                          // durable
	...
	q, err = ch.QueueDeclare(
		"update-post-detail-data-queue", // name, leave empty to generate a unique name
		true,                            // durable
	...
	err = ch.QueueBind(
		...
		"go-test-key-update-detail",   // bindingKey
		"update-post-detail-exchange", // sourceExchange
		false,                         // noWait
		nil,                           // arguments
	)
	...
	replies, err = ch.Consume(
		q.Name,                           // queue
		"go-post-update-detail-consumer", // consumer
		true,                             // auto-ack
		false,                            // exclusive
		false,                            // no-local
		false,                            // no-wait
...
func main() {
	...
	for r := range replies {
		...
		postData := models.PostDetailParamModel{}
		insertData := models.PostDetailModel{}
		...
		insertData.PostId = postData.PostId

		id := postData.DbId
		postController := v1.ServiceContainer().InjectPostController()
		_, err := postController.UpdatePostDetailProcess(id, insertData)
		if err != nil {
